...
This is a portable engineering methodology for AI coding agents. It is distilled from a real, shipped build: a hexagonal, human-in-the-loop AI agent deployed to a serverless cloud runtime behind a framework-free domain, with a CI-able eval harness gating its LLM decisions. Every rule here earned its place by surviving that build — divergence between docs and code, real-infra failures, over-flagging models, secret handling, teardown. None of it assumes a particular language, framework, or agent runtime.
...
The illustrative project (the hexagonal human-in-the-loop AI agent shipped to a serverless cloud runtime) appears throughout only as a concrete example. You never need to know anything about it to apply a rule — every principle stands on its own on any stack.
