...
8. **Capture only what earns its keep.** The trigger is "would otherwise be re-derived" — nothing else qualifies. A fact kept "in case it changes", a restated document, a status narrative: none of these meet it. Memory is a bounded cache read into a limited context, not an archive.
...
10. **Prune as you write.** Adding a note includes deleting or merging whatever it obsoletes. A store that only grows is not accumulating judgment; it is diluting it, and dragging stale vocabulary into every future task.
