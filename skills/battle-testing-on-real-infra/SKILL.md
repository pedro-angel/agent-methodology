...
2. Make live tests a first-class, separately-runnable gate, distinct from the fast unit run. Examples: a `live` test marker, a `make test-live` target, a tagged CI stage, or a dedicated script. The default suite stays fast; the live gate is explicit and on-demand. On-demand must never decay into optional: when the live gate cannot run in CI, wire it into a required checkpoint — a mandatory release/merge checklist step, with the evidence artifact traveling on the claiming commit (an `Evidence:` trailer, a committed results file). An optional gate is not a gate.
...
A third field test found the inverse failure: a client repo whose live integration suite existed but was wired into no gate at all — CI never ran it, and the release checklist listed it as "optional, if client logic changed." A commit claiming complete API coverage shipped nine integration tests that had never run green against the live server (they exercised features the server gates behind flags the tests never enabled). The gap was not missing tests; it was a live gate nothing required. The fix was as much process as code: the suite became a named, required pre-release step, and the tests were fixed against the live server the same day.
...
- Wiring the live suite into no required gate — CI never runs it and the release checklist says "optional, if things changed" — so a commit claiming full coverage ships tests that have never run.
