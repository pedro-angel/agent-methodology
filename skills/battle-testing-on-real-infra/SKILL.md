...
5. Probe external assumptions against the real endpoint before trusting docs. Hit it once and record the actual HTTP status, the real licensing/feature flag, the true model or API id, the real default limits. Believe the probe over the documentation.
