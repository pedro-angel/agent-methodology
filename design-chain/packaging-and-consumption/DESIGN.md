...
(the 10-member set, source-of-truth = the collector's `EXPECTED`; grown from 9 by SPECS v6 REQ-12):
`t_missing`, `t_partial`, `t_sha_mismatch`,
`t_force_refused`, `t_first_resolution_wins`, `t_export_fidelity_mismatch`, `t_reap_preserves_current_previous`,
`t_review_fail_closed`, `t_wiring_absent_at_provision`, `t_exclude_unknown_refused`. **Mutation
